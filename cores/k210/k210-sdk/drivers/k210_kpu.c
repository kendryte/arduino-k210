...
//#include "bsp.h"
#include "nncase.h"
//#include "../nncase/include/nncase.h"
#include "rtthread.h"
//#include "k210_kpu_cpp.h"
...
uint8_t *kpu_get_output_buf(kpu_task_t* task)
{
    ...
    return rt_malloc_align(output_size, 8);
}
...
{
    sysctl_dma_select((sysctl_dma_channel_t)task->dma_ch, SYSCTL_DMA_SELECT_AI_RX_REQ);
...
int kpu_single_task_init(kpu_task_t *task)
{
    ...
    task->dst = (uint64_t *)rt_malloc_align(task->dst_length * 8, 8);
...
static void kpu_kmodel_fully_connected(const kpu_model_fully_connected_layer_argument_t *arg, kpu_model_context_t *ctx)
{
    ...
    if(NULL == ctx->kmodel_fully_connected_buffer) {
        assert(!"Error not prepare buffer for fully_connected layer");
    }

    float *bias = (float *)ctx->kmodel_fully_connected_buffer;
    float *weights = (float *)((uint8_t *)ctx->kmodel_fully_connected_buffer + out_channels * sizeof(float));

    //rt_kprintf("kpu_kmodel_fully_connected size: %ld, %ld\n", in_channels * out_channels * sizeof(float), out_channels * sizeof(float));
...
static void kpu_conv(const kpu_model_conv_layer_argument_t *arg, kpu_model_context_t *ctx)
{
    ...
    if (arg->flags & KLF_MAIN_MEM_OUT)
    {
        ...
        if (ctx->current_layer != ctx->layers_length)
            dmac_irq_register(dma_ch, ai_step, ctx, 1);
        else
            dmac_irq_register(dma_ch, (plic_irq_callback_t)kpu_kmodel_done, ctx, 1);
        dmac_set_single_mode(dma_ch, (void *)(&kpu->fifo_data_out), dest, DMAC_ADDR_NOCHANGE, DMAC_ADDR_INCREMENT,
            DMAC_MSIZE_8, DMAC_TRANS_WIDTH_64, (layer.dma_parameter.data.dma_total_byte + 8) / 8);
    }
    else
...
static void kpu_prealloc_for_fully_conn_layer(kpu_model_context_t *ctx)
{
    kpu_model_layer_header_t *hdr = NULL;
    kpu_model_fully_connected_layer_argument_t *layer = NULL;

    const uint8_t *body = ctx->body_start;

    uint32_t ic, oc, max_ic = 0, max_oc = 0, ioc_mut = 0;

    for(uint32_t id = 0; id < ctx->layers_length; id++) {
        hdr = (kpu_model_layer_header_t *)(ctx->layer_headers + id);
        if(KL_FULLY_CONNECTED == hdr->type) {
            layer = (kpu_model_fully_connected_layer_argument_t *)body;
            ...
            //rt_kprintf("ic %d oc %d mic %d moc %d\n", ic,oc,max_ic,max_oc);
        }
        body += hdr->body_size;
    }

    (void)max_ic; // fix compiler warning.

    if(0x00 != ioc_mut) {
        //rt_kprintf("ioc_mut--:%d\n", (ioc_mut+max_oc)*sizeof(float));
        ctx->kmodel_fully_connected_buffer = (void *)rt_malloc_align((ioc_mut+max_oc)*sizeof(float), 8);
    }
}
...
int kpu_load_kmodel(kpu_model_context_t *ctx, const uint8_t *buffer)
{
    ...
    if (header->version == 3 && header->arch == 0)
    {
        ...
        ctx->kmodel_fully_connected_buffer = NULL;
        // ctx->main_buffer = (uint8_t *)malloc(header->main_mem_usage);
        ctx->main_buffer = (uint8_t *)rt_malloc_align(header->main_mem_usage, 8);
        if (!ctx->main_buffer)
            return -1;
        ...
        kpu_prealloc_for_fully_conn_layer(ctx);
...
void kpu_model_free(kpu_model_context_t *ctx)
{
    ...
    rt_free_align(ctx->main_buffer);
    ctx->main_buffer = NULL;
    if(ctx->kmodel_fully_connected_buffer){
        rt_free_align(ctx->kmodel_fully_connected_buffer);
        ctx->kmodel_fully_connected_buffer = NULL;
    }
}
