...
s32_t SPIFFS_ix_map(spiffs *fs,  spiffs_file fh, spiffs_ix_map *map,
  ...
  rt_memset(map_buf, 0, sizeof(spiffs_page_ix) * (map->end_spix - map->start_spix + 1));
  fd->ix_map = map;
...
s32_t SPIFFS_ix_remap(spiffs *fs, spiffs_file fh, u32_t offset) {
  ...
  if (spix_diff != 0) {
    ...
    if (spix_diff >= vec_len) {
      // moving beyond range
      rt_memset(&map->map_buf, 0, vec_len * sizeof(spiffs_page_ix));
      ...
      rt_memset(&map->map_buf[vec_len - spix_diff], 0, spix_diff * sizeof(spiffs_page_ix));
      // populate_ix_map is inclusive
      ...
      // memset is non-inclusive
      rt_memset(&map->map_buf[0], 0, -spix_diff * sizeof(spiffs_page_ix));
      // populate_ix_map is inclusive
