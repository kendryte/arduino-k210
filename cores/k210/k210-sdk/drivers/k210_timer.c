...
bool timer_is_enable(timer_device_number_t timer_number, timer_channel_number_t channel)
{
    volatile uint32_t ctrl = timer[timer_number]->channel[channel].control;

    return ctrl & TIMER_CR_ENABLE;
}
