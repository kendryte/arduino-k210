...
void i2s_handle_data_dma(i2s_device_number_t device_num, i2s_data_t data, plic_interrupt_t *cb)
{
    ...
    if(data.mode == I2S_TRANSMITTER) {
        configASSERT(data.send.tx_buf && data.send.tx_len);

        if(data.wait_dma_idle) {
            dmac_wait_done(data.send.tx_channel);
        }
        ...
        if(cb) {
            ...
            g_i2s_send_instance[device_num].dmac_channel = data.send.tx_channel;
            g_i2s_send_instance[device_num].transfer_mode = I2S_TRANSMITTER;
            dmac_irq_register(data.send.tx_channel, i2s_dmac_irq, &g_i2s_send_instance[device_num], cb->priority);
        }

        sysctl_dma_select((sysctl_dma_channel_t)data.send.tx_channel, SYSCTL_DMA_SELECT_I2S0_TX_REQ + device_num * 2);
        dmac_set_single_mode(data.send.tx_channel, data.send.tx_buf, (void *)(&i2s[device_num]->txdma), DMAC_ADDR_INCREMENT,
                             DMAC_ADDR_NOCHANGE, DMAC_MSIZE_1, DMAC_TRANS_WIDTH_32, data.send.tx_len);

        if(!cb && data.wait_dma_done) {
            dmac_wait_done(data.send.tx_channel);
        }
    } else {
        configASSERT(data.recv.rx_buf && data.recv.rx_len);
        if(data.wait_dma_idle) {
            dmac_wait_done(data.recv.rx_channel);
        }
        ...
        if(cb) {
            ...
            g_i2s_recv_instance[device_num].dmac_channel = data.recv.rx_channel;
            g_i2s_recv_instance[device_num].transfer_mode = I2S_RECEIVER;
            dmac_irq_register(data.recv.rx_channel, i2s_dmac_irq, &g_i2s_recv_instance[device_num], cb->priority);
        }

        sysctl_dma_select((sysctl_dma_channel_t)data.recv.rx_channel, SYSCTL_DMA_SELECT_I2S0_RX_REQ + device_num * 2);
        dmac_set_single_mode(data.recv.rx_channel, (void *)(&i2s[device_num]->rxdma), data.recv.rx_buf, DMAC_ADDR_NOCHANGE, DMAC_ADDR_INCREMENT,
                             DMAC_MSIZE_1, DMAC_TRANS_WIDTH_32, data.recv.rx_len);
