...
#include "spiffs/spiffs/spiffs.h"
#include "spiffs/spiffs/spiffs_nucleus.h"
...
static void esp_spiffs_free(esp_spiffs_t ** efs)
{
    ...
    *efs = NULL;
    ...
    rt_free_align(e->fds);
    rt_free_align(e->cache);
    rt_free_align(e->work);
    rt_free_align(e);
}
...
static esp_err_t esp_spiffs_by_label(const char* label, int * index){
    ...
    for (i = 0; i < CONFIG_SPIFFS_MAX_PARTITIONS; i++) {
        ...
        if (p) {
            ...
            if (label && p->by_label && rt_strncmp(label, p->partition->label, 17) == 0) {
                *index = i;
                return ESP_OK;
            }
        }
...
static esp_err_t esp_spiffs_init(const esp_vfs_spiffs_conf_t* conf)
{
    ...
    if (esp_spiffs_get_empty(&index) != ESP_OK) {
        LOG_E("max mounted partitions reached");
        return ESP_ERR_INVALID_STATE;
    }

    uint32_t flash_page_size = 256; //g_rom_flashchip.page_size;
    uint32_t log_page_size = CONFIG_SPIFFS_PAGE_SIZE;
    if (log_page_size % flash_page_size != 0) {
        LOG_E("SPIFFS_PAGE_SIZE is not multiple of flash chip page size (%" PRIu32 ")",
                flash_page_size);
    ...
    const esp_partition_t* partition = esp_partition_find_first(conf->partition_label);
    if (!partition) {
        LOG_E("spiffs partition could not be found");
        return ESP_ERR_NOT_FOUND;
    }

    // if (partition->encrypted) {
    //     LOG_E("spiffs can not run on encrypted partition");
    //     return ESP_ERR_INVALID_STATE;
    // }

    const size_t flash_erase_sector_size = 4096; //g_rom_flashchip.sector_size;
    ...
    if (partition->address % flash_erase_sector_size != 0) {
        LOG_E("spiffs partition is not aligned to flash sector size, please check the partition table");
    ...
    if (partition->size / flash_erase_sector_size > (spiffs_block_ix) -1) {
        LOG_E("spiffs partition is too large for spiffs_block_ix type");
        return ESP_ERR_INVALID_ARG;
    }
    if (partition->size / log_page_size > (spiffs_page_ix) -1) {
         ...
         */
        LOG_E("spiffs partition is too large for spiffs_page_ix type. Please increase CONFIG_SPIFFS_PAGE_SIZE.");
        return ESP_ERR_INVALID_ARG;
    }
    if (2 + 2 * (partition->size / (2 * log_page_size)) > (spiffs_obj_id) -1) {
        LOG_E("spiffs partition is too large for spiffs_obj_id type. Please increase CONFIG_SPIFFS_PAGE_SIZE.");
        return ESP_ERR_INVALID_ARG;
    }
    if (partition->size / log_page_size - 1 > (spiffs_span_ix) -1) {
        LOG_E("spiffs partition is too large for spiffs_span_ix type. Please increase CONFIG_SPIFFS_PAGE_SIZE.");
        return ESP_ERR_INVALID_ARG;
    }

    // esp_spiffs_t * efs = rt_calloc(1, sizeof(esp_spiffs_t));
    esp_spiffs_t * efs = rt_malloc_align(sizeof(esp_spiffs_t), 8);
    if (efs == NULL) {
        LOG_E("esp_spiffs could not be malloced");
    ...
    // if (efs->lock == NULL) {
    //     LOG_E("mutex lock could not be created");
    //     esp_spiffs_free(&efs);
    //     return ESP_ERR_NO_MEM;
    // }

    rt_mutex_init(&efs->lock, "spiffsLc", RT_IPC_FLAG_FIFO);

    efs->fds_sz = conf->max_files * sizeof(spiffs_fd);
    // efs->fds = rt_calloc(1, efs->fds_sz);
    efs->fds = rt_malloc_align(efs->fds_sz, 8);
    if (efs->fds == NULL) {
        LOG_E("fd buffer could not be allocated");
        esp_spiffs_free(&efs);
        return ESP_ERR_NO_MEM;
    }
    ...
    // efs->cache = rt_calloc(1, efs->cache_sz);
    efs->cache = rt_malloc_align(efs->cache_sz, 8);

    if (efs->cache == NULL) {
        LOG_E("cache buffer could not be allocated");
        esp_spiffs_free(&efs);
        return ESP_ERR_NO_MEM;
    }
#endif
    ...
    // efs->work = rt_calloc(1, work_sz);
    efs->work = rt_malloc_align(work_sz, 8);
    if (efs->work == NULL) {
        LOG_E("work buffer could not be allocated");
        esp_spiffs_free(&efs);
        return ESP_ERR_NO_MEM;
    }

    // efs->fs = rt_calloc(1, sizeof(spiffs));
    efs->fs = rt_malloc_align(sizeof(spiffs), 8);
    if (efs->fs == NULL) {
        LOG_E("spiffs could not be allocated");
        esp_spiffs_free(&efs);
        return ESP_ERR_NO_MEM;
    }
    ...
    static uint8_t _flash_init_flag = 0;
    if(0x00 == _flash_init_flag) {
        _flash_init_flag = 1;
        spiffs_api_init();
    }
    ...
    if (conf->format_if_mount_failed && res != SPIFFS_OK) {
        LOG_W("mount failed, %" PRId32 ". formatting...", SPIFFS_errno(efs->fs));
        SPIFFS_clearerr(efs->fs);
        res = SPIFFS_format(efs->fs);
        if (res != SPIFFS_OK) {
            LOG_E("format failed, %" PRId32, SPIFFS_errno(efs->fs));
            SPIFFS_clearerr(efs->fs);
            esp_spiffs_free(&efs);
            return ESP_FAIL;
        }
    ...
    if (res != SPIFFS_OK) {
        LOG_E("mount failed, %" PRId32, SPIFFS_errno(efs->fs));
        SPIFFS_clearerr(efs->fs);
        esp_spiffs_free(&efs);
        return ESP_FAIL;
    }
...
esp_err_t esp_spiffs_check(const char* partition_label)
{
    ...
    if (SPIFFS_check(_efs[index]->fs) != SPIFFS_OK) {
        ...
        LOG_E("SPIFFS_check failed (%d)", spiffs_res);
...
esp_err_t esp_spiffs_format(const char* partition_label)
{
    ...
    if (err != ESP_OK) {
        ...
        RT_ASSERT(err == ESP_OK && "failed to get index of the partition just mounted");
    ...
    if (res != SPIFFS_OK) {
        LOG_E("format failed, %" PRId32, SPIFFS_errno(_efs[index]->fs));
        SPIFFS_clearerr(_efs[index]->fs);
    ...
    if (partition_was_mounted) {
        ...
        if (res != SPIFFS_OK) {
            LOG_E("mount failed, %" PRId32, SPIFFS_errno(_efs[index]->fs));
            SPIFFS_clearerr(_efs[index]->fs);
            return ESP_FAIL;
        }
    } else {
...
static DIR* vfs_spiffs_opendir(void* ctx, const char* name)
{
    RT_ASSERT(name);
    esp_spiffs_t * efs = (esp_spiffs_t *)ctx;
    // vfs_spiffs_dir_t * dir = rt_calloc(1, sizeof(vfs_spiffs_dir_t));
    vfs_spiffs_dir_t * dir = rt_malloc_align(sizeof(vfs_spiffs_dir_t), 8);
...
static void vfs_spiffs_seekdir(void* ctx, DIR* pdir, long offset)
{
    ...
    while (dir->offset < offset) {
        ...
        size_t plen = rt_strlen(dir->path);
        if (plen > 1) {
...
static int vfs_spiffs_update_mtime_value(spiffs *fs, const char *path, spiffs_time_t t)
{
    ...
    if (ret == SPIFFS_OK) {
        rt_memcpy(s.meta, &t, sizeof(t));
        ret = SPIFFS_update_meta(fs, path, s.meta);
    }
    if (ret != SPIFFS_OK) {
        LOG_W("Failed to update mtime (%d)", ret);
    }
    return ret;
}
#endif //CONFIG_SPIFFS_USE_MTIME
