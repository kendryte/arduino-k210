...
spisd_result_t spisd_init(spisd_interface_t const *const io, int8_t sck, int8_t miso, int8_t mosi, int8_t cs) {
    RT_ASSERT(io);

    _io = io;

    _io->init(sck, miso, mosi, cs);

    if ( !_io->is_present() ) {
        LOG_E("There is no card detected! \r\n");
    ...
    if (!timeout) {
        LOG_E("Reset card into IDLE state failed!\r\n");
    ...
    if (response == SPISD_R1_IDLE_FLAG) {
        ...
        if (buff[2] == 0x01 && buff[3] == 0xAA) {
            ...
            if (response != 0x00) {
                LOG_E("Send ACMD41 should return 0x00, response=0x%02x\r\n", response);
                return SPISD_RESULT_TIMEOUT;
            }
            ...
            if(CARD_TYPE_SDV2HC == _card_type) {
                rt_kputs(DBG_TAG" Card Type : SD V2HC\r\n");
            } else {
                rt_kputs(DBG_TAG" Card Type : SD V2\r\n");
            }
        ...
        if (response != 0x00) {
            ...
            /* Timeout return */
            if (response != 0x00) {
                LOG_E("Send CMD1 should return 0x00, response=0x%02x\r\n", response);
                return SPISD_RESULT_TIMEOUT;
            }

            _card_type = CARD_TYPE_MMC;
            rt_kputs(DBG_TAG" Card Type : MMC\r\n");
        } else {
            rt_kputs(DBG_TAG" Card Type : SD V1\r\n");
        }
        ...
        if (response != 0x00) {
            LOG_E("Send CMD59 should return 0x00, response=0x%02x\r\n", response);
            return SPISD_RESULT_TIMEOUT;
        }
...
int spisd_get_card_info(spisd_info_t *cardinfo) {
    ...
    cardinfo->block_size = 1 << cardinfo->csd.RdBlockLen; // BLOCK_SIZE;

    if(0x00 == cardinfo->csd.CSDStruct) {
        cardinfo->capacity = (uint64_t)((cardinfo->csd.DeviceSize + 1) << (cardinfo->csd.DeviceSizeMul + cardinfo->csd.RdBlockLen + 2));
    } else {
        // csd 2.0 standard.
        cardinfo->csd.DeviceSize  = (uint32_t)(temp[7] & 63) << 16;
        cardinfo->csd.DeviceSize += (uint32_t)(temp[8]) << 8;
