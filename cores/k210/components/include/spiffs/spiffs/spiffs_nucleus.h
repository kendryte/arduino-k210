...
} __attribute(( aligned(8) )) spiffs_cache_page;
...
typedef struct {
  ...
  u8_t *cpages;
} __attribute(( aligned(8) )) spiffs_cache;
...
#endif
} __attribute(( aligned(8) )) spiffs_fd;
...
typedef struct SPIFFS_PACKED {
  ...
  // flags
  u8_t flags;
} __attribute(( aligned(8) )) spiffs_page_header;
...
#endif
} __attribute(( aligned(8) )) spiffs_page_object_ix_header;

// object index page header
typedef struct SPIFFS_PACKED {
 spiffs_page_header p_hdr;
 u8_t _align[4 - ((sizeof(spiffs_page_header)&3)==0 ? 4 : (sizeof(spiffs_page_header)&3))];
} __attribute(( aligned(8) )) spiffs_page_object_ix;
...
#define _SPIFFS_MEMCPY(__d, __s, __l) do { \
    ...
    if (__a1 <= __b2 && __b1 <= __a2) { \
      rt_kprintf("FATAL OVERLAP: memcpy from %lx..%lx to %lx..%lx\n", __a1, __a2, __b1, __b2); \
      ERREXIT(); \
    } \
    rt_memcpy((__d),(__s),(__l)); \
} while (0)
#else
#ifndef _SPIFFS_MEMCPY
#define _SPIFFS_MEMCPY(__d, __s, __l) do{rt_memcpy((__d),(__s),(__l));}while(0)
#endif
#endif //_SPIFFS_TEST
