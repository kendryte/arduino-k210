 ...
 */
void dvp_init(void);
#endif //K210_DVP_ENABLE_SCCB

/**
 * @brief       Set Camera reset pin to val
 * 
 * @param       val     pin value; normal 0 is reset, 1 is work mode.
*/
void dvp_set_rest(int val);

/**
 * @brief       Set Camera power down pin to val
 * 
 * @param       val     pin value; normal 0 is work mode, 1 is powerdown mode.
*/
void dvp_set_pwdn(int val);
 ...
 */
void dvp_set_display_addr(uint8_t *addr);
 ...
 * @param[in]   b_addr      The B address of RGB
 */
void dvp_set_ai_addr(uint8_t *r_addr, uint8_t *g_addr, uint8_t *b_addr);
