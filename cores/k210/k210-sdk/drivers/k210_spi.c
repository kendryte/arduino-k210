                            ...
                            size_t cmd_len, const uint8_t *tx_buff, size_t tx_len)
{
    ...
    uint8_t *v_buf = rt_malloc_align(cmd_len + tx_len, 8);
    size_t i;
    for(i = 0; i < cmd_len; i++)
        v_buf[i] = cmd_buff[i];
    for(i = 0; i < tx_len; i++)
                                ...
                                const uint8_t *cmd_buff, size_t cmd_len, const uint8_t *tx_buff, size_t tx_len)
{
    ...
    switch(frame_width)
    {
        case SPI_TRANS_INT:
            ...
            buf = rt_malloc_align(cmd_len + tx_len, 8);
        ...
        case SPI_TRANS_SHORT:
            ...
            buf = rt_malloc_align((cmd_len + tx_len) / 2 * sizeof(uint32_t), 8);
        ...
        default:
            ...
            buf = rt_malloc_align((cmd_len + tx_len) * sizeof(uint32_t), 8);
            for(i = 0; i < cmd_len; i++)
                buf[i] = cmd_buff[i];
            for(i = 0; i < tx_len; i++)
                              ...
                              const void *tx_buff, size_t tx_len, spi_transfer_width_t spi_transfer_width)
{
    ...
    switch(spi_transfer_width)
    {
        case SPI_TRANS_SHORT:
            ...
            buf = (uint32_t *)rt_malloc_align((tx_len) * sizeof(uint32_t), 8);
#endif
            for(i = 0; i < tx_len; i++)
                                   ...
                                   const uint8_t *tx_buf, size_t tx_len, uint8_t *rx_buf, size_t rx_len)
{
    ...
    uint32_t *v_tx_buf = rt_malloc_align(v_max_len * 4, 8);
    uint32_t *v_rx_buf = rt_malloc_align(v_max_len * 4, 8);
#endif
    uint32_t i = 0;
    switch(frame_width)
    {
    ...
    // free(v_tx_buf);
    // free(v_rx_buf);
    rt_free_align(v_tx_buf);
    rt_free_align(v_rx_buf);
#endif
}
...
void spi_transfer_data_standard(spi_device_num_t spi_num, spi_chip_select_t chip_select, const uint8_t *tx_buff, 
                                uint8_t *rx_buff, size_t len)
{
    configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    configASSERT(len > 0);
                                   ...
                                   size_t cmd_len, uint8_t *rx_buff, size_t rx_len)
{
    ...
    switch(frame_width)
    {
        case SPI_TRANS_INT:
            ...
            write_cmd = rt_malloc_align(cmd_len + rx_len, 8);
#endif
            for(i = 0; i < cmd_len / 4; i++)
        ...
        case SPI_TRANS_SHORT:
            ...
            write_cmd = rt_malloc_align((cmd_len + rx_len) / 2 * sizeof(uint32_t), 8);
#endif
            for(i = 0; i < cmd_len / 2; i++)
        ...
        default:
            ...
            write_cmd = rt_malloc_align((cmd_len + rx_len) * sizeof(uint32_t), 8);
#endif
            for(i = 0; i < cmd_len; i++)
                                ...
                                const uint32_t *cmd_buff, size_t cmd_len, const uint8_t *tx_buff, size_t tx_len)
{
    ...
    switch(frame_width)
    {
        case SPI_TRANS_INT:
            ...
            buf = rt_malloc_align(cmd_len * sizeof(uint32_t) + tx_len, 8);
#endif
            for(i = 0; i < cmd_len; i++)
                buf[i] = cmd_buff[i];
            for(i = 0; i < tx_len / 4; i++)
        ...
        case SPI_TRANS_SHORT:
            ...
            buf = rt_malloc_align(cmd_len * sizeof(uint32_t) + tx_len / 2 * sizeof(uint32_t), 8);
#endif
            for(i = 0; i < cmd_len; i++)
                buf[i] = cmd_buff[i];
            for(i = 0; i < tx_len / 2; i++)
...
static void spi_slave_command_mode(void)
{
    ...
    } else if(g_instance.command.cmd == WRITE_DATA_BLOCK)
    {
        ...
        sysctl_dma_select((sysctl_dma_channel_t)g_instance.dmac_channel, SYSCTL_DMA_SELECT_SSI0_RX_REQ + spi_num * 2);
    ...
    } else if(g_instance.command.cmd == READ_DATA_BLOCK)
    {
        ...
        sysctl_dma_select((sysctl_dma_channel_t)g_instance.dmac_channel, SYSCTL_DMA_SELECT_SSI0_TX_REQ + spi_num * 2);
...
void spi_handle_data_dma(spi_device_num_t spi_num, spi_chip_select_t chip_select, spi_data_t data, plic_interrupt_t *cb)
{
    ...
    switch(data.transfer_mode)
    {
        ...
        case SPI_TMOD_TRANS:
            ...
            sysctl_dma_select((sysctl_dma_channel_t)data.tx_channel, SYSCTL_DMA_SELECT_SSI0_TX_REQ + spi_num * 2);
            if(data.fill_mode)
