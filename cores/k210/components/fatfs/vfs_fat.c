...
static size_t find_context_index_by_path(const char* base_path)
{
    for(size_t i=0; i<FF_VOLUMES; i++) {
        if (s_fat_ctxs[i] && !rt_strcmp(s_fat_ctxs[i]->base_path, base_path)) {
            return i;
        }
    }
    return FF_VOLUMES;
...
esp_err_t esp_vfs_fat_register(const char* base_path, const char* fat_drive, size_t max_files, FATFS** out_fs)
{
    ...
    vfs_fat_ctx_t* fat_ctx = (vfs_fat_ctx_t*) rt_malloc(ctx_size);
    if (fat_ctx == NULL) {
        return ESP_ERR_NO_MEM;
    }
    rt_memset(fat_ctx, 0, ctx_size);
    fat_ctx->o_append = rt_malloc(max_files * sizeof(bool));
    if (fat_ctx->o_append == NULL) {
        rt_free(fat_ctx);
        return ESP_ERR_NO_MEM;
    }
    rt_memset(fat_ctx->o_append, 0, max_files * sizeof(bool));
                           ...
                           uint64_t* out_free_bytes)
{
    ...
    if (res != FR_OK) {
        LOG_E("Failed to get number of free clusters (%d)", res);
...
static int fresult_to_errno(FRESULT fr)
{
    ...
    RT_ASSERT(0 && "unhandled FRESULT");
    return ENOTSUP;
}

static void file_cleanup(vfs_fat_ctx_t* ctx, int fd)
{
    rt_memset(&ctx->files[fd], 0, sizeof(FIL));
}
...
static int vfs_fat_open(void* ctx, const char * path, int flags, int mode)
{
    LOG_I("%s: path=\"%s\", flags=%x, mode=%x", __func__, path, flags, mode);
    ...
    if (fd < 0) {
        ...
        LOG_E("open: no free file descriptors");
        errno = ENFILE;
        return -1;
    }
    ...
    if (res != FR_OK) {
        ...
        _lock_release(&fat_ctx->lock);
        LOG_D("%s: fresult=%d", __func__, res);
        errno = fresult_to_errno(res);
        return -1;
    }
    ...
    if(!(fat_mode_conv(flags) & (FA_WRITE))) {
        DWORD *clmt_mem =  rt_malloc(sizeof(DWORD) * CONFIG_FATFS_FAST_SEEK_BUFFER_SIZE);
        if (clmt_mem == NULL) {
            f_close(file);
            file_cleanup(fat_ctx, fd);
            _lock_release(&fat_ctx->lock);
            LOG_E("open: Failed to pre-allocate CLMT buffer for fast-seek");
            errno = ENOMEM;
            return -1;
        }
        ...
        LOG_D("%s: fast-seek has: %s",
                __func__,
                (res == FR_OK) ? "activated" : "failed");
        if(res != FR_OK) {
            LOG_W("%s: fast-seek not activated reason code: %d",
                    __func__, res);
...
static ssize_t vfs_fat_pread(void *ctx, int fd, void *dst, size_t size, off_t offset)
{
    ...
    if (f_res != FR_OK) {
        LOG_D("%s: fresult=%d", __func__, f_res);
...
#if FF_FS_EXFAT
    LOG_D("%s: offset=%ld, filesize:=%" PRIu64, __func__, new_pos, f_size(file));
#else
    LOG_D("%s: offset=%ld, filesize:=%" PRIu32, __func__, new_pos, f_size(file));
#endif
...
static int vfs_fat_link(void* ctx, const char* n1, const char* n2)
{
    ...
    FRESULT res;
    FIL* pf1 = (FIL*) rt_malloc(sizeof(FIL));
    FIL* pf2 = (FIL*) rt_malloc(sizeof(FIL));
    void* buf = rt_malloc(copy_buf_size);
    if (buf == NULL || pf1 == NULL || pf2 == NULL) {
        _lock_release(&fat_ctx->lock);
        LOG_D("alloc failed, pf1=%p, pf2=%p, buf=%p", pf1, pf2, buf);
        rt_free(pf1);
        rt_free(pf2);
        rt_free(buf);
        errno = ENOMEM;
    ...
    rt_memset(pf1, 0, sizeof(*pf1));
    rt_memset(pf2, 0, sizeof(*pf2));
    ...
    while (size_left > 0) {
        ...
        size_t read;
        res = f_read(pf1, buf, (UINT)will_copy, (UINT*)&read);
        if (res != FR_OK) {
            goto fail3;
        ...
        size_t written;
        res = f_write(pf2, buf, (UINT)will_copy, (UINT*)&written);
        if (res != FR_OK) {
            goto fail3;
...
static DIR* vfs_fat_opendir(void* ctx, const char* name)
{
    ...
    vfs_fat_dir_t* fat_dir = rt_malloc(sizeof(vfs_fat_dir_t));
    if (!fat_dir) {
        _lock_release(&fat_ctx->lock);
        errno = ENOMEM;
        return NULL;
    }
    rt_memset(fat_dir, 0, sizeof(*fat_dir));
...
static void vfs_fat_seekdir(void* ctx, DIR* pdir, long offset)
{
    ...
    if (offset < fat_dir->offset) {
        ...
        if (res != FR_OK) {
            LOG_D("%s: rewinddir fresult=%d", __func__, res);
            errno = fresult_to_errno(res);
            return;
        }
        fat_dir->offset = 0;
    ...
    while (fat_dir->offset < offset) {
        ...
        if (res != FR_OK) {
            LOG_D("%s: f_readdir fresult=%d", __func__, res);
            errno = fresult_to_errno(res);
            return;
        }
        fat_dir->offset++;
...
static int vfs_fat_truncate(void* ctx, const char *path, off_t length)
{
    ...
    if (sz < length) {
        _lock_release(&fat_ctx->lock);
        LOG_D("truncate does not support extending size");
        errno = EPERM;
        ret = -1;
        goto close;
    }
...
close:
    ...
    if (res != FR_OK) {
        LOG_E("closing file opened for truncate failed");
...
static int vfs_fat_ftruncate(void* ctx, int fd, off_t length)
{
    ...
    if (file == NULL) {
        LOG_D("ftruncate NULL file pointer");
        errno = EINVAL;
        ret = -1;
        goto out;
    }
    ...
    long sz = f_size(file);
    if (sz < length) {
        LOG_D("ftruncate does not support extending size");
        errno = EPERM;
        ret = -1;
        goto out;
    }
