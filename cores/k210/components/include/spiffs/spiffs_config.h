...
#include "rtthread.h"
#include "spiffs/k210_spiffs_config.h"

// Set generic spiffs debug output call.
#if CONFIG_SPIFFS_DBG
#define SPIFFS_DBG(fmt, ...)            do { rt_kprintf("SPIFFS_DBG->" fmt, ##__VA_ARGS__); } while(0)
#else
#define SPIFFS_DBG(...)        
#endif
#if CONFIG_SPIFFS_API_DBG
#define SPIFFS_API_DBG(fmt, ...)        do { rt_kprintf("SPIFFS_API_DBG->" fmt, ##__VA_ARGS__); } while(0)
#else
#define SPIFFS_API_DBG(...)
#endif
#if CONFIG_SPIFFS_DBG
#define SPIFFS_GC_DBG(fmt, ...)         do { rt_kprintf("SPIFFS_GC_DBG->" fmt, ##__VA_ARGS__); } while(0)
#else
#define SPIFFS_GC_DBG(...)
#endif
#if CONFIG_SPIFFS_CACHE_DBG
#define SPIFFS_CACHE_DBG(fmt, ...)      do { rt_kprintf("SPIFFS_CACHE_DBG->" fmt, ##__VA_ARGS__); } while(0)
#else
#define SPIFFS_CACHE_DBG(...)
#endif
#if CONFIG_SPIFFS_CHECK_DBG
#define SPIFFS_CHECK_DBG(fmt, ...)      do { rt_kprintf("SPIFFS_CHECK_DBG->" fmt, ##__VA_ARGS__); } while(0)
#else
