...
static struct _reent *__getreent(void)
{
    struct _reent *ret = _GLOBAL_REENT;
...
esp_err_t esp_vfs_register_common(const char* base_path, size_t len, const esp_vfs_t* vfs, void* ctx, int *vfs_index)
{
    ...
    vfs_entry_t *entry = (vfs_entry_t*) rt_malloc_align(sizeof(vfs_entry_t), 8);
    if (entry == NULL) {
    ...
    s_vfs[index] = entry;
    if (len != LEN_PATH_PREFIX_IGNORED) {
        rt_strcpy(entry->path_prefix, base_path); // we have already verified argument length
    } else {
        // bzero(entry->path_prefix, sizeof(entry->path_prefix));
        rt_memset(entry->path_prefix, 0, sizeof(entry->path_prefix));
    }
    rt_memcpy(&entry->vfs, vfs, sizeof(esp_vfs_t));
...
{
    return esp_vfs_register_common(base_path, rt_strlen(base_path), vfs, ctx, NULL);
}

esp_err_t esp_vfs_register_fd_range(const esp_vfs_t *vfs, void *ctx, int min_fd, int max_fd)
{
    if (min_fd < 0 || max_fd < 0 || min_fd > MAX_FDS || max_fd > MAX_FDS || min_fd > max_fd) {
        LOG_D("Invalid arguments: esp_vfs_register_fd_range(0x%x, 0x%x, %d, %d)", (int) vfs, (int) ctx, min_fd, max_fd);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (ret == ESP_OK) {
        ...
        for (int i = min_fd; i < max_fd; ++i) {
            if (s_fd_table[i].vfs_index != -1) {
                ...
                LOG_D("esp_vfs_register_fd_range cannot set fd %d (used by other VFS)", i);
                return ESP_ERR_INVALID_ARG;
            }
        ...
        LOG_W("esp_vfs_register_fd_range is successful for range <%d; %d) and VFS ID %d", min_fd, max_fd, index);
    }
...
{
    const size_t base_path_len = rt_strlen(base_path);
...
esp_err_t esp_vfs_register_fd_with_local_fd(esp_vfs_id_t vfs_id, int local_fd, bool permanent, int *fd)
{
    if (vfs_id < 0 || vfs_id >= s_vfs_count || fd == NULL) {
        LOG_D("Invalid arguments for esp_vfs_register_fd_with_local_fd(%d, %d, %d, 0x%p)",
    ...
    LOG_D("esp_vfs_register_fd_with_local_fd(%d, %d, %d, 0x%p) finished with %s",
...
esp_err_t esp_vfs_unregister_fd(esp_vfs_id_t vfs_id, int fd)
{
    ...
    if (vfs_id < 0 || vfs_id >= s_vfs_count || fd < 0 || fd >= MAX_FDS) {
        LOG_D("Invalid arguments for esp_vfs_unregister_fd(%d, %d)", vfs_id, fd);
        return ret;
    }
    ...
    LOG_D("esp_vfs_unregister_fd(%d, %d) finished with %s", vfs_id, fd, esp_err_to_name(ret));
...
{
    RT_ASSERT(strncmp(src_path, vfs->path_prefix, vfs->path_prefix_len) == 0);
    if (rt_strlen(src_path) == vfs->path_prefix_len) {
...
#ifdef CONFIG_LIBC_STUB_NOT_USE_ALIAS
...
#endif
{
    ...
    if (vfs == NULL || local_fd < 0) {
        if (fd == STDOUT_FILENO || fd == STDERR_FILENO) {
            extern int rt_hw_console_write(const void *buf, size_t size);
            return rt_hw_console_write(data, size);
        } else if (fd == STDIN_FILENO) {
            __errno_r(r) = ENOTSUP;
            return -1;
        } else {
            __errno_r(r) = EBADF;
            LOG_E("%s %d vfs %p fs %d\r\n", __func__, __LINE__, vfs, local_fd);
            return -1;
        }
    }
...
static void call_end_selects(int end_index, const fds_triple_t *vfs_fds_triple, void **driver_args)
{
    for (int i = 0; i < end_index; ++i) {
        ...
        if (vfs && vfs->vfs.end_select && item->isset) {
            ...
            if (err != ESP_OK) {
                LOG_D("end_select failed: %s", esp_err_to_name(err));
            }
        }
    }
}
            ...
            for (int fd = 0; fd < MAX_FDS; ++fd) {
                if (s_fd_table[fd].vfs_index == i) {
                    ...
                    if (readfds && esp_vfs_safe_fd_isset(local_fd, &item->readfds)) {
                        LOG_D("FD %d in readfds was set from VFS ID %d", fd, i);
                        FD_SET(fd, readfds);
                        ++ret;
                    }
                    if (writefds && esp_vfs_safe_fd_isset(local_fd, &item->writefds)) {
                        LOG_D("FD %d in writefds was set from VFS ID %d", fd, i);
                        FD_SET(fd, writefds);
                        ++ret;
                    }
                    if (errorfds && esp_vfs_safe_fd_isset(local_fd, &item->errorfds)) {
                        LOG_D("FD %d in errorfds was set from VFS ID %d", fd, i);
                        FD_SET(fd, errorfds);
                        ++ret;
                    }
                }
...
int esp_vfs_select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *errorfds, struct timeval *timeout)
{
    ...
    LOG_D("esp_vfs_select starts with nfds = %d", nfds);
    if (timeout) {
        LOG_D("timeout is %lds + %ldus", (long)timeout->tv_sec, timeout->tv_usec);
    }
    ...
    for (int fd = 0; fd < nfds; ++fd) {
        ...
        if (esp_vfs_safe_fd_isset(fd, readfds)) {
            ...
            FD_CLR(fd, readfds);
            LOG_D("removing %d from readfds and adding as local FD %d to fd_set of VFS ID %d", fd, local_fd, vfs_index);
        }
        if (esp_vfs_safe_fd_isset(fd, writefds)) {
            item->isset = true;
            FD_SET(local_fd, &item->writefds);
            FD_CLR(fd, writefds);
            LOG_D("removing %d from writefds and adding as local FD %d to fd_set of VFS ID %d", fd, local_fd, vfs_index);
        }
        if (esp_vfs_safe_fd_isset(fd, errorfds)) {
            item->isset = true;
            FD_SET(local_fd, &item->errorfds);
            FD_CLR(fd, errorfds);
            LOG_D("removing %d from errorfds and adding as local FD %d to fd_set of VFS ID %d", fd, local_fd, vfs_index);
        }
    }
    ...
    if (!socket_select) {
        ...
        if ((sel_sem.sem = xSemaphoreCreateBinary()) == NULL) {
            free(vfs_fds_triple);
            __errno_r(r) = ENOMEM;
            LOG_D("cannot create select semaphore");
            return -1;
        }
    }
    ...
    if (driver_args == NULL) {
        free(vfs_fds_triple);
        __errno_r(r) = ENOMEM;
        LOG_D("calloc is unsuccessful for driver args");
        return -1;
    }
    ...
    for (size_t i = 0; i < vfs_count; ++i) {
        ...
        if (vfs && vfs->vfs.start_select && item->isset) {
            ...
            LOG_D("calling start_select for VFS ID %d with the following local FDs", i);
            ...
            if (err != ESP_OK) {
                ...
                free(driver_args);
                __errno_r(r) = EINTR;
                LOG_D("start_select failed: %s", esp_err_to_name(err));
                return -1;
            }
        }
    }
    ...
    if (socket_select) {
        LOG_D("calling socket_select with the following FDs");
        esp_vfs_log_fd_set("readfds", readfds);
        esp_vfs_log_fd_set("writefds", writefds);
        esp_vfs_log_fd_set("errorfds", errorfds);
        ret = socket_select(nfds, readfds, writefds, errorfds, timeout);
        LOG_D("socket_select returned %d and the FDs are the following", ret);
        esp_vfs_log_fd_set("readfds", readfds);
        esp_vfs_log_fd_set("writefds", writefds);
        esp_vfs_log_fd_set("errorfds", errorfds);
    } else {
        ...
        if (timeout) {
            ...
            LOG_D("timeout is %" PRIu32 "ms", timeout_ms);
        }
        LOG_D("waiting without calling socket_select");
    ...
    LOG_D("esp_vfs_select returns %d", ret);
    esp_vfs_log_fd_set("readfds", readfds);
    esp_vfs_log_fd_set("writefds", writefds);
    esp_vfs_log_fd_set("errorfds", errorfds);
    return ret;
...
#ifndef CONFIG_LIBC_STUB_NOT_USE_ALIAS
int _open_r(struct _reent *r, const char * path, int flags, int mode)
...
#endif // CONFIG_LIBC_STUB_NOT_USE_ALIAS
